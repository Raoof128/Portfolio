...
- **Follow-ups**: Deploy via `npm run build && npx wrangler pages deploy out --project-name raoufabedini --branch main`. Translate the updated résumé/About/home strings into fa/ar/es/zh for locale parity of *content* (currently only structural parity). Optionally refresh the About `spec*` skill lists and `s03` proficiencies to foreground agent red-teaming.
