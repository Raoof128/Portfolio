...
> = {
  DEFENSIVE: {
    dot: "bg-cyan",
    badge: "text-cyan border-cyan/30 bg-cyan/8",
    hover: "hover:border-cyan/30",
    corner: "border-cyan/25 group-hover:border-cyan/60",
    tint: "hover:bg-cyan/[0.04]",
    prefix: "SEC",
  },
  ENGINEERING: {
    dot: "bg-purple",
    badge: "text-purple border-purple/30 bg-purple/8",
    hover: "hover:border-purple/30",
    corner: "border-purple/25 group-hover:border-purple/60",
    tint: "hover:bg-purple/[0.04]",
    prefix: "SYS",
  },
  OFFENSIVE: {
    dot: "bg-amber",
    badge: "text-amber border-amber/30 bg-amber/8",
    hover: "hover:border-amber/30",
    corner: "border-amber/25 group-hover:border-amber/60",
    tint: "hover:bg-amber/[0.04]",
    prefix: "OPS",
  },
};
...
function BentoCard({
  ...
  return (
    ...
    >
      ...
      {/* HUD corner markers */}
      <div
        className={`absolute top-0 left-0 w-5 h-5 border-t border-l ${cornerClass} transition-colors`}
      />
      <div
        className={`absolute bottom-0 right-0 w-5 h-5 border-b border-r ${cornerClass} transition-colors`}
      />
      {glowPulse && (
        <motion.div
...
export default function Home() {
  ...
  const categoryLabel = (cat: string) =>
    cat === "DEFENSIVE"
      ? t.home.category_defensive
      : cat === "OFFENSIVE"
        ? t.home.category_offensive
        : t.home.category_engineering;
  ...
  return (
    <div>
      ...
      <section className="relative min-h-screen flex items-center overflow-hidden -mt-16 pt-16">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full py-20 lg:py-28">
          ...
          >
            ...
            >
              {[t.hero.evaluator, t.hero.researcher, t.hero.university].map(
                (c) => (
                  <motion.span
                    key={c}
                    variants={fadeInUp}
                    className="px-3 py-1 border border-cyan/12 text-text-meta font-mono text-xs"
                  >
                    {c}
                  </motion.span>
                ),
            ...
            >
              <NeonButton href={getPath("/projects")} variant="primary">
                <Terminal className={cn("w-4 h-4", isRTL ? "ml-2" : "mr-2")} />{" "}
        ...
        >
          <span className="font-mono text-[9px] text-text-meta tracking-[0.4em] uppercase">
            {t.home.scroll}
          </span>
          <motion.div
      ...
      <div className="border-y border-cyan/10 bg-[#040610] py-3 overflow-hidden">
        ...
        >
          {[...t.home.ticker, ...t.home.ticker].map((item, i) => (
          ...
          <div className="flex items-end justify-between mb-10 md:mb-14 border-b border-cyan/10 pb-5">
            <div>
              <p className="font-mono text-[10px] text-text-meta tracking-[0.3em] uppercase mb-2">
                {t.home.deployed_systems}
              </p>
              <h2 className="text-3xl md:text-4xl font-bold text-white">
                <span className="text-cyan font-mono mr-3 text-glow">
                  <DecryptedText text="01." />
                </span>
                {t.nav.projects.startsWith("/")
                  ? t.nav.projects.substring(1)
                  : t.nav.projects}
              </h2>
            </div>
              ...
              return (
                ...
                >
                  <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4 mb-5">
                    <span
                      className={`font-mono text-[10px] px-2 py-0.5 border ${s.badge} tracking-widest uppercase self-start`}
                    >
                      [{s.prefix}] {categoryLabel(p.category)}
                    </span>
                    <div className="flex items-center gap-3">
                      <span className="font-mono text-[10px] text-cyan border border-cyan/30 px-2 py-0.5 tracking-widest uppercase">
                        {t.home.ieee_format_paper}
                      </span>
                  ...
                  <div className="flex flex-wrap gap-1.5 mb-6">
                    {p.tags.map((tag) => (
                      <span
                        key={tag}
                        className="font-mono text-[10px] text-text-meta border border-cyan/10 px-2 py-0.5"
                      >
                        {tag}
                      </span>
                    ))}
                ...
                >
                  <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4 mb-5">
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                      <span className="font-mono text-[10px] text-cyan border border-cyan/30 px-2 py-0.5 tracking-widest uppercase">
                        {t.home.connected_to_invisible_window}
                      </span>
                      <span className="font-mono text-[10px] text-text-meta">
                        {p.year}
                      </span>
              ...
              return (
                ...
                >
                  <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4 mb-5">
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                      <span className="font-mono text-[10px] text-purple-400 border border-purple-400/30 px-2 py-0.5 tracking-widest uppercase">
                        {t.home.local_first_mcp}
                      </span>
                      <span className="font-mono text-[10px] text-text-meta">
                        {p.year}
                      </span>
                  ...
                  <div className="flex flex-wrap gap-1.5 mb-6">
                    {p.tags.map((tag) => (
                      <span
                        key={tag}
                        className="font-mono text-[10px] text-text-meta border border-purple-400/10 px-2 py-0.5"
                      >
                        {tag}
                      </span>
                    ))}
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between mb-5">
                    <span
                      className={`font-mono text-[10px] px-2 py-0.5 border ${s.badge} tracking-widest uppercase`}
                    >
                      [{s.prefix}] {categoryLabel(p.category)}
                    </span>
                    <span className="font-mono text-[10px] text-text-meta">
      ...
      <AnimatedSection variants={fadeInUp}>
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-28">
          ...
          <div className="grid md:grid-cols-2 gap-0 border border-cyan/10">
            <AnimatedSection
              variants={fadeInLeft}
              className="border-b md:border-b-0 md:border-r border-cyan/10"
            >
              <div className="relative p-10 md:p-14 group hover:bg-cyan/[0.03] transition-colors h-full overflow-hidden">
                ...
                <motion.span
                  ...
                  style={
                    { color: "rgba(0,245,255,0.07)" } as React.CSSProperties
                  }
                >
                  R
                </motion.span>
                <div className="relative z-10">
                  ...
                  <ul className="space-y-2 font-mono text-xs text-text-meta">
                    {[
                      t.philosophy.research_bullet_1,
                      t.philosophy.research_bullet_2,
                      t.philosophy.research_bullet_3,
                    ].map((item) => (
            ...
            <AnimatedSection variants={fadeInRight}>
              <div className="relative p-10 md:p-14 group hover:bg-purple/[0.03] transition-colors h-full overflow-hidden">
                ...
                <motion.span
                  ...
                  className={cn(
                    "absolute -top-4 text-[10rem] font-bold leading-none select-none pointer-events-none",
                    isRTL ? "-left-3" : "-right-3",
                  )}
                  style={
                    { color: "rgba(139,92,246,0.07)" } as React.CSSProperties
                  }
                >
                  S
                </motion.span>
                <div className="relative z-10">
                  ...
                  <ul className="space-y-2 font-mono text-xs text-text-meta">
                    {[
                      t.philosophy.secure_bullet_1,
                      t.philosophy.secure_bullet_2,
                      t.philosophy.secure_bullet_3,
                    ].map((item) => (
                      <li key={item} className="flex items-center gap-2">
      ...
      <AnimatedSection variants={fadeInUp}>
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-28">
          <HUDFrame
            className={cn(
              "bg-gradient-to-r from-cyber-dark via-cyber-dark/60 to-transparent p-8 md:p-12",
              isRTL ? "border-r-4 border-r-amber" : "border-l-4 border-l-amber",
            )}
          >
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-8">
              <div className="space-y-5">
                ...
                <div className="flex flex-wrap gap-2">
                  {[t.lab.chip_1, t.lab.chip_2, t.lab.chip_3, t.lab.chip_4].map(
                    (lab) => (
                      <span
                        key={lab}
                        className="font-mono text-[10px] text-text-meta border border-cyan/10 px-2.5 py-1"
                      >
                        {lab}
                      </span>
                    ),
              ...
              >
                ...
                >
                  ...
                  <div className="flex items-center gap-4 shrink-0">
                    ...
                    <span className="font-mono text-xs text-text-meta tabular-nums">
                      {post.date}
                    </span>
                    <ArrowRight
                      className={cn(
                        "w-4 h-4 text-cyan/30 group-hover:text-cyan transition-colors",
                        isRTL ? "rotate-180" : "",
                      )}
                    />
                  </div>
